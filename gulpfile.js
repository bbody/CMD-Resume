...
// Build the project
gulp.task('build', ['jscs:development', 'test:karma', 'compile:development',
	'jshint:development', 'copy:html', 'copy:json', 'copy:icon']);
...
gulp.task('jscs:tools', function() {
	return gulp.src(['karma.conf.js', 'gulpfile.js'])
		.pipe(jscs())
		.pipe(jscs.reporter())
		.pipe(jscs.reporter());
});
