...
(() => {
  ...
  var updateTitle = function(name) {
    document.title = name ? name + "'s R\xE9sum\xE9" : "Command Line R\xE9sum\xE9";
  };
  ...
  var buildSocialNetwork = function(value) {
    if (value.network) {
      var address = buildSocialNetworkAddress(
        value.network,
        value.url,
        value.username
      );
  ...
  $.fn.CMDResume = function(primaryEndpoint, options) {
    ...
    self.allCommands = [
      ...
      {
        name: "Name",
        description: "owner of the r\xE9sum\xE9",
        data: ["basics", "name"],
        type: CMD.BASIC
      },
      {
      ...
      {
        name: "pdf",
        title: "R\xE9sum\xE9 PDF",
        description: "pdf version of the r\xE9sum\xE9",
        data: ["basics", "pdfLink"],
        type: CMD.CALCULATED,
        ...
        handler: function() {
          ...
          if (self.data.customSplash) {
            ...
            if (self.data.basics.name) {
              results += "Welcome to " + self.data.basics.name.setName() + "'s r\xE9sum\xE9.";
            } else {
              results += "Welcome to my r\xE9sum\xE9.";
            }
          }
      ...
      },
      {
        name: "resume",
        title: "R\xE9sum\xE9",
        description: "prints r\xE9sum\xE9",
        type: CMD.SYSTEM,
        handler: function() {
          var results = [];
          var commands = [
    ...
    self.initGithubForks = function(options2) {
      self.showForks = options2.showForks || options2.showForks === "true";
    };
    self.init = function(options2) {
      self.initGithubForks(options2);
      self.initVariables();
      self.initCommands();
      self.initSettings();
      self.initHTMLTitle();
    ...
    self.addCommand = function(command) {
      if (command.type === CMD.SYSTEM || isDefinedNotEmpty(
        self.data,
        ...
        if (command.type !== CMD.SYSTEM) {
          tempCommand.data = getDataFromArrayKey(
            self.data,
            command.data ? command.data : command.name.toLowerCase()
          );
        }
    ...
    self.initGithub = function() {
      getGithub(
        getGithubUri(self.data.basics.githubUsername),
        self.data.basics.githubUsername,
        self.showForks,
        function(result) {
